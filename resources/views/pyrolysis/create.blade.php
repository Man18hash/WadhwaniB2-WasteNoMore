...
@section('title', 'Create Pyrolysis Operation')
@section('page-title', 'Create Pyrolysis Operation')
@section('page-description', 'Start a new pyrolysis operation for plastic waste conversion')

@section('content')
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-lg shadow-sm p-6">
        <form method="POST" action="{{ route('pyrolysis.store') }}" class="space-y-6">
            @csrf
            ...
            <div>
                ...
                <select name="input_type" id="input_type" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-primary-500 focus:border-primary-500 @error('input_type') border-red-500 @enderror" required>
                    ...
                    <option value="pet" {{ old('input_type') == 'pet' ? 'selected' : '' }}>PET (Polyethylene Terephthalate)</option>
                    <option value="hdpe" {{ old('input_type') == 'hdpe' ? 'selected' : '' }}>HDPE (High-Density Polyethylene)</option>
                    <option value="pvc" {{ old('input_type') == 'pvc' ? 'selected' : '' }}>PVC (Polyvinyl Chloride)</option>
                    <option value="ldpe" {{ old('input_type') == 'ldpe' ? 'selected' : '' }}>LDPE (Low-Density Polyethylene)</option>
                    <option value="pp" {{ old('input_type') == 'pp' ? 'selected' : '' }}>PP (Polypropylene)</option>
                    <option value="ps" {{ old('input_type') == 'ps' ? 'selected' : '' }}>PS (Polystyrene)</option>
                    <option value="mixed_plastic" {{ old('input_type') == 'mixed_plastic' ? 'selected' : '' }}>Mixed Plastic Waste</option>
                </select>
                @error('input_type')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="4" 
                          ...
                          placeholder="Describe this pyrolysis operation...">{{ old('description') }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex justify-end space-x-3">
                <a href="{{ route('pyrolysis.index') }}" class="bg-gray-300 hover:bg-gray-400 text-gray-800 px-6 py-2 rounded-lg transition-colors">
                    Cancel
                </a>
                <button type="submit" class="bg-primary-600 hover:bg-primary-700 text-white px-6 py-2 rounded-lg transition-colors">
                    <i class="fas fa-save mr-2"></i>Create Operation
                </button>
            </div>
        </form>
    </div>
