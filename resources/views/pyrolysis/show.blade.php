...
@section('title', 'Pyrolysis Operation Details')
@section('page-title', 'Pyrolysis Operation Details')
@section('page-description', 'View detailed information about pyrolysis operation')
...
<div class="max-w-6xl mx-auto">
    <div class="bg-white rounded-lg shadow-sm p-6">
        <div class="flex justify-between items-start mb-6">
            <div>
                <h2 class="text-2xl font-semibold text-gray-800">{{ $pyrolysis->batch_number }}</h2>
                <p class="text-gray-600">Pyrolysis operation started on {{ $pyrolysis->start_date->format('M d, Y') }}</p>
            </div>
            <div class="flex space-x-3">
                <a href="{{ route('pyrolysis.edit', $pyrolysis) }}" class="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-lg flex items-center space-x-2 transition-colors">
                    <i class="fas fa-edit"></i>
                    <span>Edit</span>
                </a>
                <a href="{{ route('pyrolysis.index') }}" class="bg-gray-600 hover:bg-gray-700 text-white px-4 py-2 rounded-lg flex items-center space-x-2 transition-colors">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
            <!-- Basic Information -->
            <div class="space-y-4">
                <h3 class="text-lg font-semibold text-gray-800 border-b border-gray-200 pb-2">Operation Information</h3>
                
                <div class="flex justify-between items-center py-2">
                    <span class="text-sm font-medium text-gray-600">Batch Number:</span>
                    <span class="text-sm text-gray-900 font-semibold">{{ $pyrolysis->batch_number }}</span>
                </div>
                
                <div class="flex justify-between items-center py-2">
                    <span class="text-sm font-medium text-gray-600">Plastic Type:</span>
                    <span class="text-sm text-gray-900">{{ strtoupper($pyrolysis->input_type) }}</span>
                </div>
                
                <div class="flex justify-between items-center py-2">
                    <span class="text-sm font-medium text-gray-600">Input Weight:</span>
                    <span class="text-sm text-gray-900 font-semibold">{{ number_format($pyrolysis->input_weight_kg, 2) }} kg</span>
                </div>
                
                <div class="flex justify-between items-center py-2">
                    <span class="text-sm font-medium text-gray-600">Status:</span>
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $pyrolysis->status_badge }}">
                        {{ ucfirst($pyrolysis->status) }}
                    </span>
                </div>
            </div>
            ...
            <div class="space-y-4">
                ...
                <div class="flex justify-between items-center py-2">
                    ...
                    <span class="text-sm text-gray-900">{{ $pyrolysis->start_date->format('M d, Y') }}</span>
                </div>
                
                <div class="flex justify-between items-center py-2">
                    <span class="text-sm font-medium text-gray-600">Expected Completion:</span>
                    <span class="text-sm text-gray-900">{{ $pyrolysis->expected_completion_date ? $pyrolysis->expected_completion_date->format('M d, Y') : '-' }}</span>
                </div>
                
                <div class="flex justify-between items-center py-2">
                    <span class="text-sm font-medium text-gray-600">Actual Completion:</span>
                    <span class="text-sm text-gray-900">{{ $pyrolysis->actual_completion_date ? $pyrolysis->actual_completion_date->format('M d, Y') : '-' }}</span>
                </div>
                
                @if($pyrolysis->description)
                    <div class="py-2">
        ...
        <div class="mb-8">
            <div class="flex justify-between items-center mb-4">
                <h3 class="text-lg font-semibold text-gray-800">Conversion Outputs</h3>
            ...
            @if($pyrolysis->batchOutputs->count() > 0)
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @foreach($pyrolysis->batchOutputs as $output)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                        <div class="flex space-x-2">
                                            ...
                                            <form method="POST" action="{{ route('pyrolysis.outputs.destroy', [$pyrolysis, $output]) }}" class="inline" onsubmit="return confirm('Are you sure you want to delete this output?')">
                                                @csrf
                                                @method('DELETE')
            ...
            @else
                <div class="text-center py-8 text-gray-500">
                    ...
                    <p class="text-sm">Add outputs to track pyrolysis conversion results</p>
                </div>
            @endif
        </div>
    </div>
    ...
    <div class="flex items-center justify-center min-h-screen p-4">
        <div class="bg-white rounded-lg shadow-xl max-w-md w-full">
            <div class="p-6">
                ...
                <form method="POST" action="{{ route('pyrolysis.outputs.store', $pyrolysis) }}" class="space-y-4">
                    @csrf
                    ...
                    <div>
                        ...
                        <select name="output_type" id="output_type" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-primary-500 focus:border-primary-500" required>
                            ...
                            <option value="pyrolysis_oil">Pyrolysis Oil</option>
                            <option value="syngas">Syngas</option>
                            <option value="char">Char/Carbon Residue</option>
