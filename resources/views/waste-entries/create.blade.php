...
@section('title', 'Add Waste Entry')
@section('page-title', 'Add Waste Entry')
@section('page-description', 'Record a new waste input entry')

@section('content')
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-lg shadow-sm p-6">
        <form method="POST" action="{{ route('waste-entries.store') }}" class="space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="number" name="weight_kg" id="weight_kg" step="0.01" min="0.01" value="{{ old('weight_kg') }}" 
                ...
                <div>
                    ...
                    <select name="processing_technology" id="processing_technology" 
                        ...
                        <option value="anaerobic" {{ old('processing_technology') == 'anaerobic' ? 'selected' : '' }}>Anaerobic Digestion</option>
                        <option value="bsf" {{ old('processing_technology') == 'bsf' ? 'selected' : '' }}>BSF Larvae</option>
                        <option value="activated" {{ old('processing_technology') == 'activated' ? 'selected' : '' }}>Activated Carbon</option>
                        <option value="paper" {{ old('processing_technology') == 'paper' ? 'selected' : '' }}>Paper & Packaging</option>
                        <option value="pyrolysis" {{ old('processing_technology') == 'pyrolysis' ? 'selected' : '' }}>Pyrolysis</option>
                    </select>
