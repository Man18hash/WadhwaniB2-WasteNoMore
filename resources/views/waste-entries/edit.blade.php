        ...
        <form method="POST" action="{{ route('waste-entries.update', $wasteEntry) }}" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="waste_type" id="waste_type" 
                        ...
                        <option value="paper" {{ old('waste_type', $wasteEntry->waste_type) == 'paper' ? 'selected' : '' }}>Paper</option>
                    </select>
                    @error('waste_type')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
            ...
            <div>
                ...
                <textarea name="notes" id="notes" rows="4" 
                          ...
                          placeholder="Additional notes about this waste entry... (optional)">{{ old('notes', $wasteEntry->notes) }}</textarea>
                @error('notes')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
