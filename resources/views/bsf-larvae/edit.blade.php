...
@section('title', 'Edit BSF Larvae Cycle')
@section('page-title', 'Edit BSF Larvae Cycle')
@section('page-description', 'Update BSF larvae cultivation cycle information')

@section('content')
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-lg shadow-sm p-6">
        <form method="POST" action="{{ route('bsf-larvae.update', $bsfLarvae) }}" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="batch_number" id="batch_number" value="{{ old('batch_number', $bsfLarvae->batch_number) }}" 
                           ...
                           placeholder="e.g., BSF-001" required>
                    @error('batch_number')
                ...
                <div>
                    ...
                    <input type="number" name="input_weight_kg" id="input_weight_kg" step="0.01" min="0.01" value="{{ old('input_weight_kg', $bsfLarvae->input_weight_kg) }}" 
            ...
            <div>
                ...
                <input type="text" name="input_type" id="input_type" value="{{ old('input_type', $bsfLarvae->input_type) }}" 
                       ...
                       placeholder="e.g., Fruit waste" required>
                @error('input_type')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="date" name="start_date" id="start_date" value="{{ old('start_date', $bsfLarvae->start_date->format('Y-m-d')) }}" 
                ...
                <div>
                    ...
                    <input type="date" name="expected_completion_date" id="expected_completion_date" value="{{ old('expected_completion_date', $bsfLarvae->expected_completion_date ? $bsfLarvae->expected_completion_date->format('Y-m-d') : '') }}" 
            ...
            <div>
                ...
                <select name="status" id="status" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-primary-500 focus:border-primary-500 @error('status') border-red-500 @enderror" required>
                    <option value="pending" {{ old('status', $bsfLarvae->status) == 'pending' ? 'selected' : '' }}>Pending</option>
                    <option value="processing" {{ old('status', $bsfLarvae->status) == 'processing' ? 'selected' : '' }}>Processing</option>
                    <option value="completed" {{ old('status', $bsfLarvae->status) == 'completed' ? 'selected' : '' }}>Completed</option>
                    <option value="cancelled" {{ old('status', $bsfLarvae->status) == 'cancelled' ? 'selected' : '' }}>Cancelled</option>
                </select>
                @error('status')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input type="date" name="actual_completion_date" id="actual_completion_date" value="{{ old('actual_completion_date', $bsfLarvae->actual_completion_date ? $bsfLarvae->actual_completion_date->format('Y-m-d') : '') }}" 
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="4" 
                          ...
                          placeholder="Describe this BSF larvae cultivation cycle...">{{ old('description', $bsfLarvae->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex justify-end space-x-3">
                <a href="{{ route('bsf-larvae.show', $bsfLarvae) }}" class="bg-gray-300 hover:bg-gray-400 text-gray-800 px-6 py-2 rounded-lg transition-colors">
                    Cancel
                </a>
