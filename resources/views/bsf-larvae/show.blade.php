...
@section('title', 'BSF Larvae Cycle Details')
@section('page-title', 'BSF Larvae Cycle Details')
@section('page-description', 'View detailed information about BSF larvae cycle')
...
<div class="max-w-6xl mx-auto">
    <div class="bg-white rounded-lg shadow-sm p-6">
        <div class="flex justify-between items-start mb-6">
            <div>
                <h2 class="text-2xl font-semibold text-gray-800">{{ $bsfLarvae->batch_number }}</h2>
                <p class="text-gray-600">BSF Larvae Cycle started on {{ $bsfLarvae->start_date->format('M d, Y') }}</p>
            </div>
            <div class="flex space-x-3">
                <a href="{{ route('bsf-larvae.edit', $bsfLarvae) }}" class="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-lg flex items-center space-x-2 transition-colors">
                    <i class="fas fa-edit"></i>
                    <span>Edit</span>
                </a>
                <a href="{{ route('bsf-larvae.index') }}" class="bg-gray-600 hover:bg-gray-700 text-white px-4 py-2 rounded-lg flex items-center space-x-2 transition-colors">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
            <!-- Basic Information -->
            <div class="space-y-4">
                <h3 class="text-lg font-semibold text-gray-800 border-b border-gray-200 pb-2">Cycle Information</h3>
                
                <div class="flex justify-between items-center py-2">
                    <span class="text-sm font-medium text-gray-600">Batch Number:</span>
                    <span class="text-sm text-gray-900 font-semibold">{{ $bsfLarvae->batch_number }}</span>
                </div>
                
                <div class="flex justify-between items-center py-2">
                    <span class="text-sm font-medium text-gray-600">Input Type:</span>
                    <span class="text-sm text-gray-900">{{ $bsfLarvae->input_type }}</span>
                </div>
                
                <div class="flex justify-between items-center py-2">
                    <span class="text-sm font-medium text-gray-600">Input Weight:</span>
                    <span class="text-sm text-gray-900 font-semibold">{{ number_format($bsfLarvae->input_weight_kg, 2) }} kg</span>
                </div>
                
                <div class="flex justify-between items-center py-2">
                    <span class="text-sm font-medium text-gray-600">Status:</span>
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $bsfLarvae->status_badge }}">
                        {{ ucfirst($bsfLarvae->status) }}
                    </span>
                </div>
            </div>
            ...
            <div class="space-y-4">
                ...
                <div class="flex justify-between items-center py-2">
                    ...
                    <span class="text-sm text-gray-900">{{ $bsfLarvae->start_date->format('M d, Y') }}</span>
                </div>
                
                <div class="flex justify-between items-center py-2">
                    <span class="text-sm font-medium text-gray-600">Expected Completion:</span>
                    <span class="text-sm text-gray-900">{{ $bsfLarvae->expected_completion_date ? $bsfLarvae->expected_completion_date->format('M d, Y') : '-' }}</span>
                </div>
                
                <div class="flex justify-between items-center py-2">
                    <span class="text-sm font-medium text-gray-600">Actual Completion:</span>
                    <span class="text-sm text-gray-900">{{ $bsfLarvae->actual_completion_date ? $bsfLarvae->actual_completion_date->format('M d, Y') : '-' }}</span>
                </div>
                
                @if($bsfLarvae->description)
                    <div class="py-2">
        ...
        <div class="mb-8">
            <div class="flex justify-between items-center mb-4">
                <h3 class="text-lg font-semibold text-gray-800">Cycle Outputs</h3>
            ...
            @if($bsfLarvae->batchOutputs->count() > 0)
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @foreach($bsfLarvae->batchOutputs as $output)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                        <div class="flex space-x-2">
                                            ...
                                            <form method="POST" action="{{ route('bsf-larvae.outputs.destroy', [$bsfLarvae, $output]) }}" class="inline" onsubmit="return confirm('Are you sure you want to delete this output?')">
                                                @csrf
                                                @method('DELETE')
            ...
            @else
                <div class="text-center py-8 text-gray-500">
                    ...
                    <p class="text-sm">Add outputs to track larvae production</p>
                </div>
            @endif
        </div>
    </div>
    ...
    <div class="flex items-center justify-center min-h-screen p-4">
        <div class="bg-white rounded-lg shadow-xl max-w-md w-full">
            <div class="p-6">
                ...
                <form method="POST" action="{{ route('bsf-larvae.outputs.store', $bsfLarvae) }}" class="space-y-4">
                    @csrf
                    ...
                    <div>
                        ...
                        <select name="output_type" id="output_type" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-primary-500 focus:border-primary-500" required>
                            ...
                            <option value="frass">Frass (Waste)</option>
                            <option value="adult_flies">Adult Flies</option>
                        </select>
                    </div>
