...
@section('title', 'Create BSF Larvae Cycle')
@section('page-title', 'Create BSF Larvae Cycle')
@section('page-description', 'Start a new BSF larvae cultivation cycle')

@section('content')
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-lg shadow-sm p-6">
        <form method="POST" action="{{ route('bsf-larvae.store') }}" class="space-y-6">
            @csrf
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="4" 
                          ...
                          placeholder="Describe this BSF larvae cultivation cycle...">{{ old('description') }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex justify-end space-x-3">
                <a href="{{ route('bsf-larvae.index') }}" class="bg-gray-300 hover:bg-gray-400 text-gray-800 px-6 py-2 rounded-lg transition-colors">
                    Cancel
                </a>
                <button type="submit" class="bg-primary-600 hover:bg-primary-700 text-white px-6 py-2 rounded-lg transition-colors">
                    <i class="fas fa-save mr-2"></i>Create Cycle
                </button>
            </div>
        </form>
    </div>
