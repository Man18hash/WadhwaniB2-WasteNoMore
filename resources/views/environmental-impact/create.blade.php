...
@section('title', 'Record Environmental Impact')
@section('page-title', 'Record Environmental Impact')
@section('page-description', 'Record environmental impact metrics and sustainability data')

@section('content')
<div class="max-w-4xl mx-auto">
    <div class="bg-white rounded-lg shadow-sm p-6">
        <form method="POST" action="{{ route('environmental-impact.store') }}" class="space-y-6">
            @csrf
            
            <div>
                <label for="report_date" class="block text-sm font-medium text-gray-700 mb-2">Report Date *</label>
                <input type="date" name="report_date" id="report_date" value="{{ old('report_date', now()->format('Y-m-d')) }}" 
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="number" name="waste_diverted_from_landfill_kg" id="waste_diverted_from_landfill_kg" step="0.01" min="0" value="{{ old('waste_diverted_from_landfill_kg') }}" 
                ...
                <div>
                    ...
                    <input type="number" name="co2_emissions_reduced_kg" id="co2_emissions_reduced_kg" step="0.01" min="0" value="{{ old('co2_emissions_reduced_kg') }}" 
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="number" name="renewable_energy_generated_kwh" id="renewable_energy_generated_kwh" step="0.01" min="0" value="{{ old('renewable_energy_generated_kwh') }}" 
                ...
                <div>
                    ...
                    <input type="number" name="chemical_fertilizer_replaced_kg" id="chemical_fertilizer_replaced_kg" step="0.01" min="0" value="{{ old('chemical_fertilizer_replaced_kg') }}" 
            ...
            <div>
                ...
                <input type="number" name="plastic_diverted_from_ocean_kg" id="plastic_diverted_from_ocean_kg" step="0.01" min="0" value="{{ old('plastic_diverted_from_ocean_kg') }}" 
            ...
            <div>
                ...
                <textarea name="notes" id="notes" rows="4" 
                          ...
                          placeholder="Additional notes about environmental impact...">{{ old('notes') }}</textarea>
                @error('notes')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex justify-end space-x-3">
                <a href="{{ route('environmental-impact.index') }}" class="bg-gray-300 hover:bg-gray-400 text-gray-800 px-6 py-2 rounded-lg transition-colors">
                    Cancel
                </a>
                <button type="submit" class="bg-primary-600 hover:bg-primary-700 text-white px-6 py-2 rounded-lg transition-colors">
                    <i class="fas fa-save mr-2"></i>Record Impact
                </button>
            </div>
        </form>
    </div>
