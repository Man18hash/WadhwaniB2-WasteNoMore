...
@section('title', 'Create Paper & Packaging Batch')
@section('page-title', 'Create Paper & Packaging Batch')
@section('page-description', 'Start a new paper and packaging production batch')

@section('content')
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-lg shadow-sm p-6">
        <form method="POST" action="{{ route('paper-packaging.store') }}" class="space-y-6">
            @csrf
            ...
            <div>
                ...
                <select name="input_type" id="input_type" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-primary-500 focus:border-primary-500 @error('input_type') border-red-500 @enderror" required>
                    ...
                    <option value="rice_straw" {{ old('input_type') == 'rice_straw' ? 'selected' : '' }}>Rice Straw</option>
                    <option value="corn_husk" {{ old('input_type') == 'corn_husk' ? 'selected' : '' }}>Corn Husk</option>
                    <option value="banana_fiber" {{ old('input_type') == 'banana_fiber' ? 'selected' : '' }}>Banana Fiber</option>
                    <option value="coconut_coir" {{ old('input_type') == 'coconut_coir' ? 'selected' : '' }}>Coconut Coir</option>
                    <option value="mixed_agricultural" {{ old('input_type') == 'mixed_agricultural' ? 'selected' : '' }}>Mixed Agricultural Waste</option>
                </select>
                @error('input_type')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="4" 
                          ...
                          placeholder="Describe this paper & packaging production batch...">{{ old('description') }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex justify-end space-x-3">
                <a href="{{ route('paper-packaging.index') }}" class="bg-gray-300 hover:bg-gray-400 text-gray-800 px-6 py-2 rounded-lg transition-colors">
                    Cancel
                </a>
