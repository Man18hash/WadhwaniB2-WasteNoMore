...
@section('title', 'Paper & Packaging Batch Details')
@section('page-title', 'Paper & Packaging Batch Details')
@section('page-description', 'View detailed information about paper & packaging batch')
...
<div class="max-w-6xl mx-auto">
    <div class="bg-white rounded-lg shadow-sm p-6">
        <div class="flex justify-between items-start mb-6">
            <div>
                <h2 class="text-2xl font-semibold text-gray-800">{{ $paperPackaging->batch_number }}</h2>
                <p class="text-gray-600">Paper & Packaging batch started on {{ $paperPackaging->start_date->format('M d, Y') }}</p>
            </div>
            <div class="flex space-x-3">
                <a href="{{ route('paper-packaging.edit', $paperPackaging) }}" class="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-lg flex items-center space-x-2 transition-colors">
                    <i class="fas fa-edit"></i>
                    <span>Edit</span>
                </a>
                <a href="{{ route('paper-packaging.index') }}" class="bg-gray-600 hover:bg-gray-700 text-white px-4 py-2 rounded-lg flex items-center space-x-2 transition-colors">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
            ...
            <div class="space-y-4">
                ...
                <div class="flex justify-between items-center py-2">
                    ...
                    <span class="text-sm text-gray-900 font-semibold">{{ $paperPackaging->batch_number }}</span>
                </div>
                
                <div class="flex justify-between items-center py-2">
                    <span class="text-sm font-medium text-gray-600">Input Type:</span>
                    <span class="text-sm text-gray-900">{{ ucfirst(str_replace('_', ' ', $paperPackaging->input_type)) }}</span>
                </div>
                
                <div class="flex justify-between items-center py-2">
                    <span class="text-sm font-medium text-gray-600">Input Weight:</span>
                    <span class="text-sm text-gray-900 font-semibold">{{ number_format($paperPackaging->input_weight_kg, 2) }} kg</span>
                </div>
                
                <div class="flex justify-between items-center py-2">
                    <span class="text-sm font-medium text-gray-600">Status:</span>
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $paperPackaging->status_badge }}">
                        {{ ucfirst($paperPackaging->status) }}
                    </span>
                </div>
            </div>
            ...
            <div class="space-y-4">
                ...
                <div class="flex justify-between items-center py-2">
                    ...
                    <span class="text-sm text-gray-900">{{ $paperPackaging->start_date->format('M d, Y') }}</span>
                </div>
                
                <div class="flex justify-between items-center py-2">
                    <span class="text-sm font-medium text-gray-600">Expected Completion:</span>
                    <span class="text-sm text-gray-900">{{ $paperPackaging->expected_completion_date ? $paperPackaging->expected_completion_date->format('M d, Y') : '-' }}</span>
                </div>
                
                <div class="flex justify-between items-center py-2">
                    <span class="text-sm font-medium text-gray-600">Actual Completion:</span>
                    <span class="text-sm text-gray-900">{{ $paperPackaging->actual_completion_date ? $paperPackaging->actual_completion_date->format('M d, Y') : '-' }}</span>
                </div>
                
                @if($paperPackaging->description)
                    <div class="py-2">
        ...
        <div class="mb-8">
            ...
            @if($paperPackaging->batchOutputs->count() > 0)
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @foreach($paperPackaging->batchOutputs as $output)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                        <div class="flex space-x-2">
                                            ...
                                            <form method="POST" action="{{ route('paper-packaging.outputs.destroy', [$paperPackaging, $output]) }}" class="inline" onsubmit="return confirm('Are you sure you want to delete this output?')">
                                                @csrf
                                                @method('DELETE')
            ...
            @else
                <div class="text-center py-8 text-gray-500">
                    ...
                    <p class="text-sm">Add outputs to track paper & packaging production</p>
                </div>
            @endif
        </div>
    </div>
    ...
    <div class="flex items-center justify-center min-h-screen p-4">
        <div class="bg-white rounded-lg shadow-xl max-w-md w-full">
            <div class="p-6">
                ...
                <form method="POST" action="{{ route('paper-packaging.outputs.store', $paperPackaging) }}" class="space-y-4">
                    @csrf
                    ...
                    <div>
                        ...
                        <select name="output_type" id="output_type" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-primary-500 focus:border-primary-500" required>
                            ...
                            <option value="paper_sheets">Paper Sheets</option>
                            <option value="packaging_boxes">Packaging Boxes</option>
